...
const Report = () => {
  ...
  useEffect(() => {
    fetchData();
    // simple polling to keep reports up-to-date
    const id = setInterval(fetchData, 10000);
    return () => clearInterval(id);
  }, [fetchData]);

  // Flatten dailyData into a single list of expense entries with date
  // Helper to parse en-GB date string 'DD/MM/YYYY' into Date
  const parseEnGB = (s) => {
    if (!s) return null;
    const parts = s.split("/");
  ...
  // Flatten expenses (exclude income entries) within optional date range
  const flattenExpenses = (range = null) => {
    ...
    Object.entries(dailyData).forEach(([date, items]) => {
      ...
      Object.entries(items).forEach(([key, val]) => {
        // Skip income records when building expense entries
  ...
  // Compute total income (honors date range)
  ...
  // Update filteredEntries whenever dailyData or date filters change
  useEffect(() => {
  ...
  // Compute monthly expenses list
  const getMonthKey = (d) => `${d.getFullYear()}-${d.getMonth() + 1}`;

  // Determine month to show: fromDate if set, otherwise current month
  const selectedMonth = (() => {
  ...
  // Compute totals per category for charts
  ...
  // Insights: highest category, lowest category, average daily spend
