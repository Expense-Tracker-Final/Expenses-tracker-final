...
function WeeklyMonthlySummary() {
  ...
  const [lastMonthSavings, setLastMonthSavings] = useState(null); // For last month custom summary

  // Fetch daily records and calculate current month income/balance
  useEffect(() => {
    async function fetchRecords() {
      try {
        ...
        // Calculate current month income and balance permanently
  ...
  const calculateSummary = (rangeTitle, customStart, customEnd) => {
    ...
    Object.entries(dailyRecords).forEach(([dateStr, items]) => {
      ...
      if (recordDate >= customStart && recordDate <= customEnd) {
        Object.entries(items).forEach(([key, item]) => {
          if (item.category === "income") return; // skip income for summary
    ...
    // Calculate last month savings if the selected range matches last month
    const today = new Date();
